...
public class IssueCreatedMetadataParserTest extends JiraParserV2Test<IssueCreatedMetadataParser> {
  ...
  private static final String EXPECTED_TEMPLATE_FILE = "<messageML>\n"
      ...
      + "                        <span>${entity['jiraIssue'].issue.description}</span>\n"
      + "                    </div>\n"
      + "                    <br/>\n"
      + "                    <div>\n"
      ...
      + ".emailAddress}\" />\n"
      + "                        <#else>\n"
      + "                            <span>${entity['jiraIssue'].issue.assignee"
      + ".displayName}</span>\n"
      + "                        </#if>\n"
      + "                    </div>\n"
      + "                    <hr/>\n"
      ...
      + "                            <img src=\"${entity['jiraIssue'].issue.issueType"
      + ".iconUrl}\" class=\"icon\" />\n"
      + "                            <span>${entity['jiraIssue'].issue.issueType"
      + ".name}</span>\n"
      + "                            <span class=\"tempo-text-color--secondary\">Priority:</span>\n"
      + "                            <img src=\"${entity['jiraIssue'].issue.priority"
      + ".iconUrl}\" class=\"icon\" />\n"
      + "                            <span>${entity['jiraIssue'].issue.priority"
      + ".name}</span>\n"
      + "                            <#if (entity['jiraIssue'].issue.epic)??>\n"
      + "                                <span class=\"tempo-text-color--secondary\">Epic:</span>\n"
      + "                                "
      + "<a href=\"${entity['jiraIssue'].issue.epic.link}\">${entity['jiraIssue']"
      + ".issue.epic.name}</a>\n"
      + "                            </#if>\n"
      + "                            <span class=\"tempo-text-color--secondary\">Status:</span>\n"
      + "                            <span class=\"infoBackground "
      + "badge\">${entity['jiraIssue'].issue.status}</span>\n"
      + "                            <#if (entity['jiraIssue'].issue.labels)??>\n"
      + "                                <span "
      + "class=\"tempo-text-color--secondary\">Labels:</span>\n"
